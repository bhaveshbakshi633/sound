...
make test       # C++ self-test + headless node byte-loopback through the browser's WASM
make test-browser  # Playwright end-to-end in real Chromium (needs npm + `npx playwright install chromium`)
```

Requires `libasound2-dev`. Note `-march=native` and `-ffast-math` in `CXXFLAGS`. The two `test`
targets are described under **The browser app** and **Tests** below; neither reproduces real
speaker→air→microphone acoustics.
...
Two front-ends live in `web/`, both driving the same WASM build of the DSP
(`web/uchat.js` + `web/uchat.wasm`):

- `web/index.html` — the page `uchat` serves on `127.0.0.1:8080`. The C++ host does all the
  audio; this page only renders bubbles and talks to it over SSE.
- `web/mesh.html` — a browser-only mesh PWA that captures and plays the audio itself, with no
  native host in the path.

### Capture on the audio thread (AudioWorklet)

`web/mesh.html` used to capture through `ScriptProcessorNode`. That API is deprecated, and worse,
its callback runs on the **main** thread: every batch of samples the UI had to stop what it was
doing to hand audio to the DSP, so any page jank — a reflow, a GC pause — dropped microphone
frames, and dropped frames are dropped symbols are lost messages. Capture now runs in an
`AudioWorkletProcessor` (`web/capture-worklet.js`) on the audio render thread, which the page
cannot block. The WASM DSP still runs on the main thread; the worklet's only job is to forward
samples across its port, batched to ~2048 to keep the message rate down (~16× fewer posts than
the 128-sample quantum).

### The mic is captured only while you are looking at the page

The microphone is enabled only while the page is both focused and visible
(`document.hasFocus() && document.visibilityState === 'visible'`). Switch tabs, minimise, or blur
the window and the mic track is disabled and the worklet stops forwarding — nothing is captured
while you are not looking. It is a privacy default, not a performance tweak. The `AudioContext`
keeps running so a transmission already in flight still plays out; only input is gated.

### Activity log

Every message sent or received is logged with a timestamp and persisted per-room in
`localStorage` (capped at 500 entries), restored on reload or rejoin behind a
"— N earlier message(s) · live below —" divider, and exportable as a plain-text transcript
(an **export** button; **clear** wipes it). System notices — noise floor, warnings — are shown
but **not** persisted: the log is the conversation, not the diagnostics. Writes are debounced but
flushed synchronously on `pagehide`/`blur`, so a message you just sent survives closing the tab
immediately after. `web/index.html` got the same log, export/clear, and honest SSE reconnect
status ("connection lost — retrying…" / "reconnected").

### Accessibility and mobile

ARIA roles on the live regions (`log` / `status`, and `alert` for a gate error), input labels,
44px+ tap targets, `:focus-visible` outlines, `prefers-reduced-motion` handling, `enterkeyhint`
on inputs, and a handler for the mic being unplugged or its permission revoked mid-session.

### Offline (service worker)

`web/sw.js` v1 cached `index.html` but **not** `mesh.html` — the very page that registers the
worker — nor the AudioWorklet module nor `icon.svg`, so the mesh PWA silently failed to load
offline. v2 caches the mesh app's full dependency closure and bumps the cache name to force the
re-cache on the next load that has a network.

---

## Tests

`tests/` holds two harnesses, wired into `package.json` and the `Makefile`.

- **`tests/dsp-loopback.cjs`** — a headless node harness that drives the **same WASM the browser
  loads** through the exact pack → modulate → push → pop → unpack path: a binary-unsafe high room
  hash, unicode, a near-max 200 B payload, and AWGN. Run `make test` (which also runs the C++
  self-test) or `node tests/dsp-loopback.cjs`.
- **`tests/browser.mjs`** — Playwright end-to-end in real Chromium. Chromium's fake audio device
  is fed a WAV generated by the **app's own modulator** carrying a real frame, so the browser
  genuinely decodes an ultrasonic burst end to end (worklet → WASM → UI). Covers page load, WASM
  init, AudioWorklet capture, decode into a "them" bubble, focus-gate pause/resume, activity-log
  persistence across reload, export, and the `index.html` SSE path. Run `make test-browser` (needs
  `npm install && npx playwright install chromium`).

Last run: C++ self-test all-pass, 9/9 node loopback, 15/15 Playwright.

**None of these tests reproduce real acoustics.** The fake-audio path feeds a WAV straight into
the decoder — no speaker moves air, no microphone hears it. They exercise the
pack/modulate/demodulate/unpack chain and the browser plumbing around it; they do **not** prove a
message survives a real room. The only test of the genuine speaker → air → microphone round trip
is the on-device one — a real phone or laptop in a room — exactly the 96%-over-air measurement at
the top of this file. Do not read a green Playwright run as proof of over-the-air reception.
...
| `web/index.html` | The native-host chat page served over SSE. No build step, no dependencies. |
| `web/mesh.html` | Browser-only mesh PWA: captures and plays audio itself, no native host. |
| `web/uchat.js`, `web/uchat.wasm` | WASM build of the DSP the browser pages load. |
| `web/capture-worklet.js` | AudioWorklet mic capture on the audio thread (replaces `ScriptProcessorNode`). |
| `web/sw.js`, `web/manifest.json`, `web/icon.svg` | Offline service worker + PWA install metadata. |
| `tests/dsp-loopback.cjs` | Headless node byte-loopback through the browser's WASM. No acoustics. |
| `tests/browser.mjs` | Playwright E2E in real Chromium, fake-audio WAV fed to the decoder. No acoustics. |
